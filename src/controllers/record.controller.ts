...
//       const hasAccess = allowed.some(p => p.startsWith(token));
//       if (hasAccess) {
...
export const createRecord = async (req: Request, res: Response) => {
  try {
    ...
    if (!departmentKey || !subDepartmentKey || !title || !data) {
      return res.status(400).json({ message: "departmentKey, subDepartmentKey, title and data are required" });
    }

    // 🔐 BULLETPROOF VISIBILITY ENFORCEMENT
    if (req.user.role !== "admin") {
      const allowed = req.user.visibleSubDepartments || [];
      const targetPermission = `${departmentKey}:${subDepartmentKey}:write`.toLowerCase();
      
      const hasWriteAccess = allowed.some((p: string) => p.toLowerCase() === targetPermission);
      
      if (!hasWriteAccess) {
        return res.status(403).json({ message: "Access denied. You do not have write permissions for this section." });
      }
    }

    const subDept = await SubDepartment.findOne({ departmentKey, key: subDepartmentKey });
    if (!subDept) return res.status(404).json({ message: "Sub-department not found" });
...
export const listRecords = async (req: Request, res: Response) => {
  try {
    ...
    // 🔐 BULLETPROOF MONGOOSE FILTER
    if (req.user.role !== "admin") {
      const allowed = req.user.visibleSubDepartments || [];
      
      if (allowed.length === 0) {
        return res.json([]); // Return empty immediately if no permissions
      }

      // Convert ["dept:sub:write", "dept2:sub2:read"] into MongoDB $or queries
      const orConditions = allowed.map((p: string) => {
        const parts = p.split(":");
        // Match department and sub-department ignoring the read/write tag
        return { 
          departmentKey: new RegExp(`^${parts[0]}$`, 'i'), // Case insensitive
          subDepartmentKey: new RegExp(`^${parts[1]}$`, 'i')
        };
      });
 ...
 * UPLOAD DOCUMENTS
 */
export const uploadRecordDocuments = async (req: any, res: Response) => {
  try {
    if (!req.user) return res.status(401).json({ message: "Unauthorized" });
    ...
    if (req.user.role !== "admin") {
      const allowed = req.user.visibleSubDepartments || [];
      const targetPermission = `${record.departmentKey}:${record.subDepartmentKey}:write`.toLowerCase();
      
      const hasWriteAccess = allowed.some((p: string) => p.toLowerCase() === targetPermission);
      if (!hasWriteAccess) return res.status(403).json({ message: "Access denied." });
    }

    const subDept = await SubDepartment.findOne({ departmentKey: record.departmentKey, key: record.subDepartmentKey });
    if (!subDept) return res.status(404).json({ message: "Schema not found" });
...
export const deleteRecord = async (req: Request, res: Response) => {
  try {
    ...
    await Record.findByIdAndDelete(id);
    res.json({ message: "Record and files deleted successfully", id });
  } catch (error) {
    console.error("DELETE RECORD ERROR:", error);
    res.status(500).json({ message: "Internal server error" });
  }
